...
`api-config.js` contém a URL pública do Apps Script usado pela instalação atual. Como HTML estático não lê variáveis de ambiente sozinho, não coloque segredos reais em `.env` esperando que eles apareçam no navegador.

### `apps-script/Code.gs`

É o backend opcional para a planilha existente. A aba ativa usa as colunas `pet`, `servico`, `data` e `hora`; `doGet()` lista as linhas e `doPost()` grava reservas usando `LockService` para reduzir conflitos simultâneos.
...
- Para uma implantação vendável de pequeno porte, use a planilha existente e preencha `api-config.js` com a URL publicada do Apps Script.
